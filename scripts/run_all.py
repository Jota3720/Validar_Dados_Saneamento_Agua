...
def run_pipeline(config_path: str | Path = "config/project.yaml", *, skip_extract: bool = False) -> Path:
    ...
    if not skip_extract:
        _run_extraction(
            script_name="01_extract_layers.py",
            config_path=_resolve_path(cfg.get("layers_mapping", "config/layers_mapping.yaml")),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Executa o pipeline de validação de saneamento em modo seguro.")
